...
Champs standards également écrits à la création (non `x_studio_*`, pour mémoire) : `name`, `default_code`, `list_price`, `type`, `image_1920`, `description` (lien vers la fiche VSF). Le stock n'est jamais écrit par le widget : il reste géré par les mécanismes internes Odoo.
...
`x_studio_eurocode` est affiché sous la référence interne sur la fiche produit. La référence constructeur reste affichée dans l'onglet "VSF" ajouté par `views/product_product_views.xml` (hérite de `product.product_normal_form_view`).
