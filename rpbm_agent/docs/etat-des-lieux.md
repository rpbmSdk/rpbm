...
- **`baseEurocodeField` non surchargé par modèle** (`utils.js:26`, valeur par défaut `x_studio_base_eurocode`) : contrairement à `immatriculationField`, ce champ n'est pas redéfini dans `AgentWidgetDialogCrmLead`. Or `x_studio_base_eurocode` n'existe que sur `sale.order` (champ `related` vers `opportunity_id.x_studio_field_ORIyy`) — sur `crm.lead`, le vrai champ "Base Eurocode" s'appelle `x_studio_field_ORIyy`. Conséquence : l'écriture de l'eurocode échoue silencieusement quand le widget est placé sur une Piste/Opportunité (cf. [parcours utilisateur](fonctionnel/parcours-utilisateur.md#structure-des-champs-eurocode-sur-crmlead) pour le détail des 3 champs Eurocode existants sur `crm.lead`).
...
- **Champs Studio non versionnés** : la configuration métier (quels champs existent, quel est leur type, leurs relations) vit uniquement en base de données de chaque instance, jamais dans le dépôt — reproductibilité et traçabilité des changements de configuration métier non garanties (voir [configuration](technique/configuration.md)).
...
- **Documentation "Chemin" vide** pour chaque champ Studio dans le README fonctionnel actuel — signe que l'emplacement exact des champs dans les vues n'est tracé nulle part, ce qui complique la maintenance/recréation de la configuration Studio sur une nouvelle instance.
...
2. Formaliser la création des champs `x_studio_*` manquants via un `post_init_hook` idempotent créant les `ir.model.fields` avec `context={'studio': True}` (mécanisme du mixin `web_studio`, vérifié dans le code source Odoo — voir [configuration](technique/configuration.md#mécanisme-retenu-pour-un-script-de-pré-installation)) plutôt que des champs Python classiques du module, qui seraient supprimés avec leurs données à la désinstallation.
