...
- **Route mal nommée** `/rbm_agent/getVehiculeMeta` — **corrigé** : route canonique `/rpbm_agent/getVehiculeMeta` ; l'alias temporaire a été retiré le 2026-09-20 (un seul appelant depuis L1.1).
...
- **Fermeture VSF** : `VSFAgent` n'a pas de méthode `close()` et `/rpbm_agent_close` ne ferme que X'Glass — voulu : VSF ne partage pas la contrainte "un seul utilisateur actif par identifiant", un logout explicite n'est pas nécessaire au même titre. Le commentaire mort `# vsfAgent.close()` a été retiré (2026-09-20).
...
- **`getPieceData`/`getPiecesData`** dupliqués — **corrigé (2026-09-20)** : la variante objets (`XGlassPlanche`, `XGlassPieceAm`, `getPlancheData`, `affichagePieces`, `getPieces`, `getPieceAm`) n'était appelée que par le bloc `__main__` ; les deux ont été supprimés, seule `getPiecesData` (consommée par `/getPieces`) reste.
- **`requirements.txt` partiellement incohérent** — **corrigé** : `external_dependencies` du manifest = `beautifulsoup4`, `requests` (le module ne charge plus `.env`) ; `python-dotenv` ne sert qu'aux scripts standalone (`debug_portals.py`, `push_credentials.py`).
- **Bloc `if __name__ == "__main__"`** de `xglass.py` — **retiré (2026-09-20)**, [`debug_portals.py`](../debug_portals.py) couvre le même scénario avec traçage.
...
- **Absence de sécurité fine** : pas de `security/ir.model.access.csv` ni de groupe dédié — toutes les routes sont accessibles à n'importe quel utilisateur Odoo connecté. **Décision assumée** (roadmap L3.1), pas une dette.

## 4. Cohérence technique — frontend

- **Appels réseau dupliqués** : `/getOdooVehicule` est appelé par chaque `VehiculeComponent` affiché puis une fois par `AgentWidgetDialog` à la confirmation, sans partage d'état. Atténué (2026-09-20) : la route ne renvoie plus que `name`/`driver_id` (plus d'image base64), et `/createVehicule` renvoie `{id, name}` sans relecture.
...
- **Refactoring inachevé visible** (`onConfirm()`/`onWillStart()` des sous-classes n'appelant que `super`, code commenté) — **corrigé** (L3.4 puis 2026-09-20) : le hook `onWillStart` est branché dans la base `AgentWidgetDialog`, les sous-classes ne surchargent plus que les noms de champs.
